...
class EventHandler(TranscriptResultStreamHandler):
    ...
    async def handle_transcript_event(self, transcript_event: TranscriptEvent):
        ...
        printer(f"[DEBUG] Received transcript event: {results}", "debug")
...
class MicStream:
    ...
    async def mic_stream(self):
        ...
        def callback(indata, frame_count, time_info, status):
            printer(f"[DEBUG] Received audio chunk: {len(indata)} bytes", "debug")
    ...
    async def basic_transcribe(self):
        printer("[DEBUG] Starting transcription", "debug")
